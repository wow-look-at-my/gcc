...
#include <deque>
#include <list>
namespace bigtu_extra {
template <typename T> struct Wrap { T v; T get() const { return v; } };
template <typename K, typename V> double churn() {
  std::map<K, V> m; std::set<K> s; std::deque<V> d; std::list<V> l;
  for (int i = 0; i < 16; ++i) { m[K(i)] = V(i); s.insert(K(i * 2)); d.push_back(V(i)); l.push_back(V(i)); }
  double acc = 0; for (auto &kv : m) acc += double(kv.second); return acc + double(s.size() + d.size() + l.size());
}
double drive() {
  std::regex re("^[a-z]+([0-9]{2,4})$", std::regex::extended);
  std::smatch sm; std::string probe = "abc1234";
  double r = std::regex_match(probe, sm, re) ? 1.0 : 0.0;
  r += churn<int, double>() + churn<long, float>() + churn<unsigned, long long>();
  Wrap<std::string> w{probe}; r += double(w.get().size());
  return r;
}
}
int main() { return int(bigtu_extra::drive()) & 0; }
